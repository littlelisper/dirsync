...
def ls(dir_path, files = [], dirs = []): #Lists directories recursively
...
def snip_path(dir_path, paths): #Used to shorten links, needs to be rechecked
    length = len(dir_path)
    return [path[length+1:] for path in paths]

def dirs_sync(parent, dirs): #Creates empty sub directories if they aren't present
    for dir_path in dirs:
        path = os.path.join(parent, dir_path)
        if not (os.path.exists(path)):
            os.makedirs(path)

def files_sync(dir1_path, dir2_path, files): #Syncs files based on last modified date
...
def sync(dir1_path, dir2_path): #Main Function
...
def extract_paths(paths_file_stream): #Extracting paths from a file
